...
class TestSM2Update(unittest.TestCase):
    def test_sm2_update_quality_below_3(self):
        ...
        chunk = Chunk(id=1, user_id="user1", jp_prompt="jp", en_answer="en", ef=2.5, interval=10, review_count=2)
    ...
    def test_sm2_update_quality_3(self):
        ...
        chunk = Chunk(id=1, user_id="user1", jp_prompt="jp", en_answer="en", ef=2.5, interval=10, review_count=3)
        ...
        self.assertEqual(updated_chunk.interval, int(round(10 * chunk.ef))) # Previous interval * new EF
        self.assertEqual(updated_chunk.next_due_date, date.today() + timedelta(days=updated_chunk.interval))

    def test_sm2_update_quality_5_first_review(self):
        """Test SM-2 update for a perfect score on the first review."""
        chunk = Chunk(id=1, user_id="user1", jp_prompt="jp", en_answer="en", ef=2.5, interval=0, review_count=0)
    ...
    def test_sm2_update_quality_4_second_review(self):
        ...
        chunk = Chunk(id=1, user_id="user1", jp_prompt="jp", en_answer="en", ef=2.6, interval=1, review_count=1)
    ...
    def test_sm2_update_quality_5_third_review(self):
        ...
        chunk = Chunk(id=1, user_id="user1", jp_prompt="jp", en_answer="en", ef=2.6, interval=6, review_count=2)
        updated_chunk = sm2_update(chunk, 5) # Quality = 5
        ...
        self.assertEqual(updated_chunk.next_due_date, date.today() + timedelta(days=expected_interval))

    def test_ef_floor(self):
        """Test that EF does not go below 1.3."""
        chunk = Chunk(id=1, user_id="user1", jp_prompt="jp", en_answer="en", ef=1.3, interval=10, review_count=3)
        ...
        chunk.ef = 1.35
        updated_chunk = sm2_update(chunk, 3) # Quality 3, EF = 1.35 + (0.1 - 2*0.08 - 2*0.02*2) = 1.35 + (0.1 - 0.16 - 0.08) = 1.35 - 0.14 = 1.21 -> clamped to 1.3
...
class TestChunk(unittest.TestCase):
    def test_chunk_defaults(self):
        ...
        chunk = Chunk(id=None, user_id="user1", jp_prompt="こんにちは", en_answer="Hello")
    ...
    def test_save_new_chunk(self, mock_connect):
        ...
        self.assertEqual(saved_chunk.created_at, original_created_at) # created_at should not change
        # updated_at is set to created_at by default, and not changed further on initial save
        self.assertAlmostEqual(saved_chunk.updated_at, original_created_at, delta=timedelta(seconds=1)) 
    ...
    def test_save_existing_chunk(self, mock_connect):
        ...
        chunk = Chunk(id=1, user_id="user1", jp_prompt="jp_old", en_answer="en_old",
                      created_at=initial_created_at, updated_at=initial_updated_at)
        
        chunk.jp_prompt = "jp_new" # Modify the chunk
        
        # Ensure save method updates 'updated_at'
        with patch('chunk_recall_trainer.chunk.datetime', wraps=datetime) as mock_datetime:
            ...
            self.assertEqual(saved_chunk.created_at, initial_created_at) # created_at should not change
            self.assertEqual(saved_chunk.updated_at, current_time_for_update) # updated_at should be new time
    ...
    def test_from_row(self):
        ...
        mock_sql_row = {
            "id": 1, "user_id": "user1", "jp_prompt": "jp", "en_answer": "en",
            "ef": 2.0, "interval": 5, "next_due_date": "2023-01-10",
            "review_count": 3, "created_at": "2023-01-01T10:00:00Z",
        ...
        self.assertEqual(chunk.created_at, datetime(2023, 1, 1, 10, 0, 0, tzinfo=timezone.utc))


class TestChunkRepo(unittest.TestCase):
    def setUp(self):
        """Setup an in-memory SQLite database for each test."""
        self.conn = sqlite3.connect(TEST_DB_PATH, detect_types=sqlite3.PARSE_DECLTYPES | sqlite3.PARSE_COLNAMES)
    ...
    def test_add_and_get_all_chunks(self):
        ...
        self.assertEqual(all_chunks[0].jp_prompt, chunk2_data["jp_prompt"]) # Ordered by created_at DESC
    ...
    def test_get_overdue_chunks(self):
        ...
        chunk1 = Chunk(id=None, user_id=self.user_id, jp_prompt="c1", en_answer="a1", next_due_date=yesterday)
        chunk2 = Chunk(id=None, user_id=self.user_id, jp_prompt="c2", en_answer="a2", next_due_date=today)
        chunk3 = Chunk(id=None, user_id=self.user_id, jp_prompt="c3", en_answer="a3", next_due_date=tomorrow)
    ...
    def test_delete_many_chunks(self):
        ...
        c1 = self.repo.add(Chunk(id=None, user_id=self.user_id, jp_prompt="j1", en_answer="e1"))
        c2 = self.repo.add(Chunk(id=None, user_id=self.user_id, jp_prompt="j2", en_answer="e2"))
        c3 = self.repo.add(Chunk(id=None, user_id=self.user_id, jp_prompt="j3", en_answer="e3"))
    ...
    def test_reset_intervals(self):
        ...
        c1 = self.repo.add(Chunk(id=None, user_id=self.user_id, jp_prompt="j1", en_answer="e1", interval=10, review_count=5))
        c2 = self.repo.add(Chunk(id=None, user_id=self.user_id, jp_prompt="j2", en_answer="e2", interval=5, review_count=2))
    ...
    def test_save_from_csv(self, mock_read_csv):
        ...
        all_chunks = self.repo.get_all()
        self.assertEqual(len(all_chunks), 2)
        self.assertEqual(all_chunks[0].jp_prompt, "ありがとう") # Ordered by created_at DESC by get_all
    ...
    def test_export_all(self):
        ...
        self.repo.add(Chunk(id=None, user_id=self.user_id, jp_prompt="jp1", en_answer="en1", ef=2.0, interval=1, next_due_date=date(2023,1,1)))
        self.repo.add(Chunk(id=None, user_id=self.user_id, jp_prompt="jp2", en_answer="en2", ef=2.1, interval=2, next_due_date=date(2023,1,2)))
    ...
    def test_reset_repo(self):
        ...
        self.repo.add(Chunk(id=None, user_id=self.user_id, jp_prompt="j1", en_answer="e1"))
        self.repo.add(Chunk(id=None, user_id="other_user", jp_prompt="j_other", en_answer="e_other")) # Add chunk for another user
