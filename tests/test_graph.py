...
with patch("src.chunk_recall_trainer.graph.ChatOpenAI", new=mock_chat_open_ai_class):
    ...
    mock_llm_instance.with_structured_output.return_value = (
        MagicMock()
    )  # This mock will be `llm_ex` and `llm_fb`
...
class TestGraphLogic(unittest.TestCase):
    ...
    def test_generate_exercise_node_generates(self):
        ...
        with (
            patch("src.chunk_recall_trainer.graph.llm_ex") as mock_graph_llm_ex,
            patch("src.chunk_recall_trainer.graph.PromptTemplate") as mock_prompt,
        ):
            mock_prompt_instance = MagicMock()
            mock_prompt.from_template.return_value = mock_prompt_instance

            chain_mock = MagicMock()
            chain_mock.invoke.return_value = ExerciseSchema(
                question="Generated Q", answer_key="Generated AK"
            )
            mock_prompt_instance.__or__.return_value = chain_mock
            ...
            self.assertEqual(result_state["jp_prompt"], "こんにちは")
    ...
    def test_review_output_node_generates_feedback(self):
        ...
        with (
            patch("src.chunk_recall_trainer.graph.llm_fb") as mock_graph_llm_fb,
            patch("src.chunk_recall_trainer.graph.PromptTemplate") as mock_prompt,
        ):
            mock_prompt_instance = MagicMock()
            mock_prompt.from_template.return_value = mock_prompt_instance
            ...
            chain_mock = MagicMock()
            chain_mock.invoke.return_value = ReviewSchema(
                score=3, better="Hola", comment="Good try"
            )
            mock_prompt_instance.__or__.return_value = chain_mock
            ...
            chain_mock.invoke.assert_called_once_with(
                {"en_chunk": "Hello", "user_answer": "Hallo", "model_answer": "Hello"}
            )
    ...
    def test_review_output_node_error_if_missing_input(self):
        ...
        with patch("src.chunk_recall_trainer.graph.llm_fb") as mock_graph_llm_fb:
            ...
            self.assertEqual(
                result_state_1["feedback"],
                "Error: User input or model answer key missing for review.",
            )
            mock_graph_llm_fb.invoke.assert_not_called()

            result_state_2 = review_output_node(state_no_ak)
            self.assertEqual(
                result_state_2["feedback"],
                "Error: User input or model answer key missing for review.",
            )
            mock_graph_llm_fb.invoke.assert_not_called()
    ...
    @patch("src.chunk_recall_trainer.graph.PromptTemplate")
    @patch("src.chunk_recall_trainer.graph.llm_ex")
    @patch("src.chunk_recall_trainer.graph.llm_fb")
    def test_graph_app_question_generation_only(
        self, mock_llm_fb_in_graph, mock_llm_ex_in_graph, mock_prompt
    ):
        ...
        chain_mock = MagicMock()
        chain_mock.invoke.return_value = ExerciseSchema(
            question="Test Q", answer_key="Test AK"
        )
        mock_prompt_instance.__or__.return_value = chain_mock
        ...
        self.assertIsNone(result.get("feedback"))
        chain_mock.invoke.assert_called_once()

    @patch("src.chunk_recall_trainer.graph.PromptTemplate")
    @patch("src.chunk_recall_trainer.graph.llm_ex")
    @patch("src.chunk_recall_trainer.graph.llm_fb")
    def test_graph_app_feedback_generation(
        self, mock_llm_fb_in_graph, mock_llm_ex_in_graph, mock_prompt
    ):
        ...
        chain_mock = MagicMock()
        chain_mock.invoke.return_value = ReviewSchema(
            score=4, better=None, comment="OK"
        )
        mock_prompt_instance.__or__.return_value = chain_mock
