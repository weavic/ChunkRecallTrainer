...
from chunk_recall_trainer.auth import initialize_auth, authenticate_user, render_logout_button
...
class TestAuth(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_app_config.firebase_config = {"apiKey": "test_key", "authDomain": "test_domain"}
        self.mock_app_config.allowed_emails_list = ["test@example.com", "another@example.com"]
    ...
    def test_initialize_auth(self, MockFirebaseAuth):
        ...
        self.assertIsInstance(auth_instance, MockFirebaseAuth)
    ...
    def test_authenticate_user_session_exists_allowed(self, mock_st):
        ...
        mock_st.sidebar.info.assert_called_once_with(f"Welcome back, {mock_user['email']}!")
        mock_st.sidebar.success.assert_called_once_with(f"Signed in as {mock_user['email']}")
    ...
    def test_authenticate_user_session_exists_not_allowed(self, mock_st):
        ...
        mock_auth.check_session.assert_called_once()
        mock_st.sidebar.info.assert_called_once_with(f"Welcome back, {mock_user['email']}!")
        mock_st.sidebar.error.assert_called_once_with("Access Denied: Your email is not authorized for this application.")
    ...
    def test_authenticate_user_login_form_allowed(self, mock_st):
        ...
        mock_st.sidebar.success.assert_called_once_with(f"Successfully logged in as {mock_login_user['email']}")
        mock_st.stop.assert_not_called()
    ...
    def test_authenticate_user_login_form_fails(self, mock_st):
        ...
        mock_auth.check_session.assert_called_once()
        mock_auth.login_form.assert_called_once()
        mock_st.sidebar.info.assert_called_once_with("Enter your credentials to access the application.")
        mock_st.stop.assert_called_once()
    ...
    def test_render_logout_button_stable_and_clicked(self, mock_st):
        ...
        mock_st.sidebar.button.assert_called_once_with("Logout", use_container_width=True, key="logout_button")
        mock_auth.logout_form.assert_called_once()
        mock_st.sidebar.success.assert_called_once_with("You have been successfully logged out.")
        self.assertNotIn("user_id", mock_st.session_state) # Check if user_id is deleted
    ...
    def test_render_logout_button_not_stable(self, mock_st):
        ...
        mock_st.sidebar.info.assert_called_once_with("Logout functionality is currently marked as unavailable.")
