...
from chunk_recall_trainer.config import (
    app_config,
...
if current_user:
    ...
    if "user_id" not in st.session_state or not st.session_state.get("user_id"):
        ...
        st.sidebar.error(
            "Critical: Authentication process did not complete. Please try again."
        )
...
if "user_id" in st.session_state and st.session_state.user_id:
    try:
        ...
        logger.error(
            f"Fatal Error: Could not initialize user data repository for user {st.session_state.user_id}. Details: {e}"
        )
        st.error(
            f"Fatal Error: Could not initialize user data repository. Details: {e}"
        )
    ...
    logger.error(
        "Fatal Error: User ID is missing after authentication. Cannot initialize data."
    )
    st.error(
        "Fatal Error: User ID is missing after authentication. Cannot initialize data."
    )
...
with st.sidebar:
    ...
    if (
        ...
        uploaded_csv_file = st.file_uploader(
            "Import Chunks from CSV", type="csv", key="csv_uploader"
        )
        ...
        with st.form("add_chunk_form", clear_on_submit=True):
            ...
            new_jp_prompt_input = st.text_area(
                "Japanese Prompt (JP)", height=80, key="new_jp_prompt_text_area"
            )
            new_en_answer_input = st.text_area(
                "English Answer/Chunk (EN)", height=80, key="new_en_answer_text_area"
            )
            ...
            if add_chunk_submit_button:
                if new_jp_prompt_input and new_en_answer_input:  # Basic validation
                    logger.info(
                        f"Adding new chunk: JP='{new_jp_prompt_input}', EN='{new_en_answer_input}'"
                    )
                    try:
                    ...
                    st.sidebar.warning(
                        "Both Japanese and English fields are required to add a new chunk."
                    )
        ...
        with st.form("reset_database_form"):
            st.markdown(
                "This will delete **all** your chunks. This action cannot be undone."
            )
            confirm_reset_checkbox = st.checkbox(
                "Yes, I understand and wish to delete all my chunks.",
                key="confirm_reset_db_checkbox",
            )
    ...
    user_provided_api_key = st.text_input(
        ...
        ),  # Display current session key or configured key
        ...
        ),  # Disable input if key is pre-loaded from app_config (env/secrets)
    )
    # If user enters a new key and it's different from the current session key, update it.
    if user_provided_api_key and user_provided_api_key != st.session_state.get(
        "api_key"
    ):
        st.session_state["api_key"] = (
            user_provided_api_key  # Update the API key in session state
        )
...
if "user_id" in st.session_state and st.session_state.user_id and "repo" in locals():
    ...
    if (
        "queue_date" not in st.session_state
        or st.session_state.queue_date != date.today()
    ):
        try:
            # Fetch up to 5 overdue chunks to form the daily practice queue.
            st.session_state.queue = repo.get_overdue(limit=5)
            st.session_state.queue_date = (
                date.today()
            )  # Mark queue as updated for today
            st.session_state.queue_total = len(
                st.session_state.queue
            )  # Total items for today's session
            logger.info(
                f"Practice queue updated for user {st.session_state.user_id}. Found {len(st.session_state.queue)} items."
            )
        except Exception as e:
...
current_progress_display_count = (
    total_chunks_for_today_session
    - num_remaining_in_practice_queue
    + (
        1
        if num_remaining_in_practice_queue > 0 and total_chunks_for_today_session > 0
        else 0
    )
)
...
with progress_metric_column:
    if total_chunks_for_today_session > 0:
        st.metric(
            ...
            f"{current_progress_display_count}/{total_chunks_for_today_session}",
        )
    else:
...
tab_practice_section, tab_manage_chunks_section = st.tabs(
    ["💬 Practice Exercises", "📋 Manage Your Chunks"]
)
...
with tab_practice_section:
    ...
    if not st.session_state.get("api_key"):
        ...
        st.warning(
            "⚠️ OpenAI API key is not set. Please configure it in the sidebar under Settings to use practice features."
        )
    ...
    if not practice_chunks_queue:  # Checks the daily limited queue
        st.success(
            "🎉 All chunks for today's session are reviewed! Check back tomorrow or add new chunks."
        )
    ...
    for (
        chunk_item_to_practice
    ) in (
        repo.get_overdue()
    ):  # Iterates all overdue, not just the daily `practice_chunks_queue`
        chunk_id_as_str = str(
            chunk_item_to_practice.id
        )  # Use string ID for session state keys for robustness.

        # Expander title shows Japanese prompt and next due date for context.
        with st.expander(
            f"🇯🇵 JP: {chunk_item_to_practice.jp_prompt} (Next due: {chunk_item_to_practice.next_due_date.strftime('%Y-%m-%d')})"
        ):
            st.markdown(
                f"🇬🇧 EN: {chunk_item_to_practice.en_answer}"
            )  # Display the English answer/chunk.

            # "Generate Practice Question" button:
            # Invokes the LangGraph app (`graph_app`) to generate a question and its model answer key.
            if st.button(
                "📝 Generate Practice Question", key=f"practice_btn_{chunk_id_as_str}"
            ):
                logger.info(
                    f"Generating practice question for chunk ID: {chunk_id_as_str}"
                )
                ...
                try:
                    ...
                    question_generation_result = graph_app.invoke(
                        graph_input_for_question,
                        config={
                            "run-name": f"generate_exercise_chunk-{chunk_id_as_str}"
                        },
                    )
                    ...
                    st.session_state[f"q_{chunk_id_as_str}"] = (
                        question_generation_result.get("question")
                    )
                    st.session_state[f"ak_{chunk_id_as_str}"] = (
                        question_generation_result.get("answer_key")
                    )
                    ...
                    logger.error(
                        f"Error generating question for chunk ID {chunk_id_as_str}: {e}"
                    )
            ...
            if current_practice_question:
                ...
                audio_file_upload = st.file_uploader(
                    "Upload your spoken answer (WAV or MP3 format)",
                    type=["wav", "mp3"],
                    key=f"audio_uploader_{chunk_id_as_str}",
                )
                if audio_file_upload and st.session_state.get(
                    "api_key"
                ):  # Check for API key again before OpenAI call
                    ...
                    with st.spinner("Transcribing your audio answer... Please wait."):
                        try:
                            ...
                            if transcription_response:
                                ...
                                st.session_state[f"ans_val_{chunk_id_as_str}"] = str(
                                    transcription_response
                                ).strip()
                                logger.info(
                                    f"Audio transcribed successfully for chunk ID: {chunk_id_as_str}"
                                )
                                st.success(
                                    "Audio transcribed successfully! Your answer has been populated below."
                                )
                        except Exception as e:
                            logger.error(
                                f"Audio transcription failed for chunk ID {chunk_id_as_str}: {e}"
                            )
                ...
                user_typed_answer = st.text_area(
                    ...
                    value=st.session_state.get(
                        f"ans_val_{chunk_id_as_str}", ""
                    ),  # Default to empty or previous value
                )
                ...
                if st.button(
                    "✅ Check My Answer",
                    key=f"check_btn_{chunk_id_as_str}",
                    disabled=not user_typed_answer.strip(),
                ):
                    ...
                    question_context_for_review = st.session_state.get(
                        f"q_{chunk_id_as_str}"
                    )
                    answer_key_context_for_review = st.session_state.get(
                        f"ak_{chunk_id_as_str}"
                    )

                    if (
                        not question_context_for_review
                        or not answer_key_context_for_review
                    ):
                        st.error(
                            "⚠️ Cannot check answer: The practice question or its model answer key was not found. Please generate a question first."
                        )
                    else:
                        ...
                        try:
                            feedback_result = graph_app.invoke(
                                graph_input_for_feedback,
                                config={
                                    "run-name": f"review_answer_chunk-{chunk_id_as_str}"
                                },  # LangSmith run name
                            )
                            # Store the AI-generated feedback in session state.
                            st.session_state[f"fb_{chunk_id_as_str}"] = (
                                feedback_result.get("feedback")
                            )
                        except Exception as e:
                            logger.error(
                                f"Error getting feedback for chunk ID {chunk_id_as_str}: {e}"
                            )
                ...
                current_feedback_markdown = st.session_state.get(
                    f"fb_{chunk_id_as_str}"
                )
...
with tab_manage_chunks_section:
    ...
    if not all_user_chunks_list:
        st.info(
            "You haven't added any chunks yet. Use the 'Add a New Chunk' form in the sidebar or import a CSV file!"
        )
    else:
        ...
        try:
            ...
            AttributeError
        ):  # Fallback if .model_dump() is not available (e.g. not Pydantic)
            logger.warning(
                "Using __dict__ for DataFrame conversion as model_dump is not available."
            )
    ...
    with right:
        if st.button("💾 Save edits", use_container_width=True):
            try:
                ...
                logger.info(
                    f"Bulk update successful for user {st.session_state.user_id}."
                )
                st.success("Saved!")
            except Exception as e:
                logger.error(
                    f"Bulk update failed for user {st.session_state.user_id}: {e}"
                )
        ...
        with col_del:
            ...
            if st.button("🗑 Delete", disabled=sel_id is None, use_container_width=True):
                try:
                    ...
                    logger.info(
                        f"Deleted chunk with ID {sel_id} for user {st.session_state.user_id}."
                    )
                    st.rerun()
                except Exception as e:
                    logger.error(
                        f"Failed to delete chunk with ID {sel_id} for user {st.session_state.user_id}: {e}"
                    )
        ...
        with col_reset:
            ...
            if st.button(
                ...
                try:
                    ...
                    logger.info(
                        f"Reset interval for chunk ID {sel_id} for user {st.session_state.user_id}."
                    )
                    st.rerun()
                except Exception as e:
                    logger.error(
                        f"Failed to reset interval for chunk ID {sel_id} for user {st.session_state.user_id}: {e}"
                    )
    ...
    col2.metric(
        "Avg EF", round(chunks_df["ef"].mean(), 2) if not chunks_df.empty else 0
    )
